    ...
    <section class="text-white">
        <div class="container mx-auto space-y-4 py-8 text-white">
            ...
            <div class="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <nuxt-link v-for="link in filtered_links" :key="link.title" :to="link.link">
                    <div class="h-full rounded-xl bg-black bg-opacity-70 drop-shadow-lg hover:ring-1 hover:ring-white">
                        <div class="min-h-28 px-4 pt-4">
                            <h3 class="pb-2 text-xl font-bold">{{ link.title }}</h3>
                            <div class="line-clamp-2 text-base font-light" v-html="link.description"></div>
                        </div>
                        <div v-if="showImages">
                            <img class="h-48 w-full rounded-b-xl object-cover"
